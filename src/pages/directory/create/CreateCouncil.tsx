...
const CreateCouncil = () => {
  ...
  const onSubmit = async (values: typeof initialValues) => {
    try {
      const councilRef = values.name.toLowerCase().replace(' ', '-')
      const response = await fetch(
        DIRECTORY_FUNCTION_BASE_URL + '/church/council',
        {
          method: 'POST',
          body: JSON.stringify({
            ...values,
      ...
      clickCard(councilRef, 'council')

      navigate('/council/profile')
    } catch (e: unknown) {
      if (e instanceof Error) {
        toast({
          title: 'An error occurred creating council',
  ...
  return (
    <Container>
      <Heading>Create A Council</Heading>
