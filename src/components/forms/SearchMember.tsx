...
const SearchMember = (props: RoleBasedSearch) => {
  ...
  const memberSearch = async ({
    ...
    try {
      setLoading(true)
      const fetchUrl = new URL(SEARCH_FUNCTION_BASE_URL + '/member')
      ...
      toast({
        title: 'Error searching for member',
        description: 'There was an error searching for member ' + error,
        status: 'error',
        duration: 6000,
        isClosable: true,
      })
  ...
  useEffect(() => {
    // Function to perform the member search
  ...
  return (
    <FormControl>
      ...
      <Autosuggest
        ...
        getSuggestionValue={(suggestion: Member) =>
