...
const SearchCouncil = (props: RoleBasedSearch) => {
  ...
  const councilSearch = async ({
    ...
    try {
      setLoading(true)
      const fetchUrl = new URL(SEARCH_FUNCTION_BASE_URL + '/council')
      ...
      toast({
        title: 'Error searching for council',
        description: 'There was an error searching for council ' + error,
        status: 'error',
        duration: 6000,
        isClosable: true,
      })
  ...
  useEffect(() => {
    // Function to perform the council search
    const performCouncilSearch = async () => {
      if (!noSearch) {
        await councilSearch({
          uid: user.id,
